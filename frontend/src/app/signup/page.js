...
import { Mail, Lock, Eye, EyeOff, PawPrint, ArrowRight, User, Dog } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.user.role === 'sitter') router.push('/sitter-dashboard');
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-500 relative items-center justify-center p-12">
        <div className="relative z-10 text-center">
          ...
          <h1 className="text-4xl font-bold font-[Outfit] text-white mb-4">Join PetConnect</h1>
          <p className="text-lg text-white/80 max-w-md">Create your account and start connecting with trusted pet care professionals today.</p>
          <div className="mt-12 grid grid-cols-2 gap-4 max-w-sm mx-auto">
            {[{ n: '10K+', l: 'Sitters' }, { n: '50K+', l: 'Happy Pets' }, { n: '4.9', l: 'Rating' }, { n: '100K+', l: 'Bookings' }].map((s, i) => (
              <div key={i} className="bg-white/10 backdrop-blur rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-white font-[Outfit]">{s.n}</p>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12 bg-white overflow-y-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          ...
          <h2 className="text-3xl font-bold font-[Outfit] text-slate-900 mb-2">Create Account</h2>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button type="button" onClick={() => setForm({ ...form, role: 'owner' })}
              className={`p-4 rounded-xl border-2 text-center transition-all ${form.role === 'owner' ? 'border-indigo-500 bg-indigo-50 shadow-md' : 'border-slate-200 hover:border-slate-300'}`}>
              <div className="text-2xl mb-1">🐕</div>
              <p className={`text-sm font-semibold ${form.role === 'owner' ? 'text-indigo-700' : 'text-slate-600'}`}>Pet Owner</p>
              <p className="text-xs text-slate-400">Find a sitter</p>
            </button>
            <button type="button" onClick={() => setForm({ ...form, role: 'sitter' })}
              className={`p-4 rounded-xl border-2 text-center transition-all ${form.role === 'sitter' ? 'border-purple-500 bg-purple-50 shadow-md' : 'border-slate-200 hover:border-slate-300'}`}>
              <div className="text-2xl mb-1">💼</div>
              <p className={`text-sm font-semibold ${form.role === 'sitter' ? 'text-purple-700' : 'text-slate-600'}`}>Pet Sitter</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                <input type="password" required value={form.confirmPassword} onChange={(e) => setForm({ ...form, confirmPassword: e.target.value })} className="input-field !pl-11" placeholder="••••••••" />
              </div>
            </div>

            <label className="flex items-start gap-2 text-sm text-slate-500 cursor-pointer">
              <input type="checkbox" required className="w-4 h-4 rounded border-slate-300 text-indigo-600 mt-0.5" />
              <span>I agree to the <Link href="#" className="text-indigo-600 font-medium">Terms of Service</Link> and <Link href="#" className="text-indigo-600 font-medium">Privacy Policy</Link></span>
            </label>

            <button type="submit" disabled={loading} className="w-full btn-primary justify-center !py-3.5 text-base disabled:opacity-60">
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <>Create Account <ArrowRight className="w-5 h-5" /></>}
            </button>
          </form>
